...
class GetContent(object):
    ...
    def getUrls(self):
        ...
        base_url = r'https://medlineplus.gov/ency/encyclopedia_{}.htm'
    ...
    def spider(self, urls):
        ...
        for url in urls:
            ...
            # print(soup)
            tags = soup.find_all('ul',id='index')##找到每个子链接
                ...
                for tag in tags:
                    ...
                    for li in lis:
                        if li.a['href']!=None:
                            item = ContentItem()
                            href = li.a['href']
                            link = 'https://medlineplus.gov/ency/'+href
                            content = self.getSubUrl(link)
                            if content:
                                name = li.a.string
                                item.link = link
                                item.name = re.sub('[<>/\|:*?]','',str(name))
                                item.fold_letter = url[-5]
    ...
    def getSubUrl(self,url):
        ...
        if htmlcontent != None:
            ...
            patt1 = re.compile(r'<p>(.*?)</p>|<li>(.*?)</li>|<h2>(.*?)</h2>|<h3>(.*?)</h3>', re.S)
            patt2 = re.compile('<[^>]*>')
            chapter_mains = soup.find('div',id="d-article")
            chapter_mains = patt1.findall(str(chapter_mains))
    ...
    def piplines(self, item):
        folder = r'F:\\大学项目\\illness\\English2\\'+item.fold_letter+'\\'##保存路径
